...
class Datastore
{
    public function get(string $source) : ?array
    {
        ...
        try {
            ...
            (new Logger)->error("get/datastore/error", ["source"=>$source, "error"=>$e->getMessage() ?? ""]);
        }
        return [
    ...
    public function set(string $source, string $destination) : bool
    {
        ...
        try {
            ...
            (new Logger)->error("set/datastore/error", ["source"=>$source,"destination"=>$destination, "error"=>$e->getMessage() ?? ""]);
            return false;
        }
        return true;
    }
